...
thread_tick (void) 
{
  ...
  if(thread_mlfqs && timer_ticks()>0){
    if (!(thread_current() == idle_thread || thread_current() == mang_thread || thread_current() == recent_cpu_thread))
      thread_current ()->recent_cpu = _ADD_INT (thread_current ()->recent_cpu, 1);
